...
static void	bmp_meta_info(t_all *all)
{
	...
	all->screenshot.bfh[3] = (unsigned char)
			(all->screenshot.size_of_file >> 8);
	all->screenshot.bfh[4] = (unsigned char)
			(all->screenshot.size_of_file >> 16);
	all->screenshot.bfh[5] = (unsigned char)
			(all->screenshot.size_of_file >> 24);
